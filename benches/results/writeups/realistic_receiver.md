...
mattered in any run: the largest jump any packet demanded was 10 keys 
...
|-------------|-------------|---------|------|------|-------|-------|
| epoch       | clean       |   316.1 |  333 |  375 |   459 | 36.75 |
| epoch       | disturbed   |   304.2 |  292 |  375 |   458 | 38.18 |
| frame       | clean       |   315.1 |  333 |  375 |   459 | 36.87 |
| frame       | disturbed   |   305.5 |  292 |  375 |   625 | 38.02 |
| packet      | clean       |  1134.0 | 1125 | 1250 |  1666 | 10.24 |
| packet      | disturbed   |  1091.6 |  667 | 3250 |  4042 | 10.64 |
...
- epoch: 36.7 Gbps clean, 38.2 Gbps disturbed
- frame: 36.9 Gbps clean, 38.0 Gbps disturbed
- packet: 10.2 Gbps clean, 10.6 Gbps disturbed

Two observations:

- Packet-level keying costs ~3.6x: it sustains ~3.6x fewer packets
...
- **Frame:** only the extreme tail moves: p99.9 increases from 459 to
  625 ns. The expensive packets are the *flip-flops*: a straggler from
  ...
  classification (section 3) counts them: 670 straggler calls at
  ~512 ns mean, versus ~304 ns for a normal packet, plus roughly as many
  swap-back cases. Together ~1,340 of the 950,000 measured calls
  (~0.14%) are made expensive by flip-flopping. p99.9 is the cost at the border of the slowest
  0.1% of calls. Since 0.14% > 0.1%, the slowest 0.1% consists entirely of
  flip-flop calls.
- **Packet:** the whole distribution changes. The median goes down and
  ...
  1,134 ns (p50 = 1125, p99 = 1250).
  Under disturbance there are two kinds of calls, counted by the
  per-call classification of the next section. 61% of the calls belong to
  packets that arrive late, after a jump-ahead call already derived
  their key (575,328 of the 950,000 measured calls, from the class
  table of section 3). In this case we just read the key and decrypt.
  It costs 642 ns, which is cheaper than any clean call. The other 39%
  (374,672 calls) are the jump-ahead calls themselves. Each of them derives every key
  that its jump skipped, which costs 1,773 ns on average. This explains 
  the two factors in the table:  1) the median drops from 1125 to 667 because 
  the cheap kind makes up 61% of all calls, so the call in the middle of the distribution is a cheap
  one, and 2) p99 goes from 1250 to 3250 and p99.9 from 1666 to 4042
...
|-----------|----------------------------------|---------|---------|--------|
| advance   | derived new key(s) (catch-up)    | 374,672 | 1,772.7 |  3,625 |
| straggler | late packet, reuses old key      | 575,328 |   642.0 |    833 |
...
|-------|---------|---------|-|-------|--------|---------|
| 1     | 120,823 |   1,144 | | 6     | 12,253 |   3,231 |
| 2     |  95,449 |   1,547 | | 7     |  5,015 |   3,630 |
| 3     |  68,859 |   1,957 | | 8     |  1,581 |   4,040 |
| 4     |  44,796 |   2,358 | | 9     |    377 |   4,669 |
| 5     |  25,468 |   2,819 | | 10    |     51 |   5,136 |

The cost grows linearly with the keys derived, about 410 ns per key.
...
values against this table. The slowest 1% of all calls must be calls of
depth 6 or more, because the n column for depths 7 to 10 sums to 7,024
calls (0.74% of the 950,000) and adding depth 6 overshoots to 2.0%, so
the 1% border falls among the depth-6 calls. The measured p99 is
consistent with that: 3,250 ns sits right at the depth 6 mean
(3,231 ns). The same works for the p99.9: depths 9 and 10 sum to only
428 calls (0.045%) and adding depth 8 overshoots to 0.21%, so the 0.1%
border falls among the depth-8 calls, and the measured p99.9 of
4,042 ns sits right at the depth 8 mean (4,040 ns).
...
|-----------|---------|---------|--------|
| advance   |     261 | 1,322.0 |  3,667 |
| current   | 949,069 |   303.5 |    375 |
| straggler |     670 |   511.8 |    750 |

Here the "current" class dominates: 99.9% of the calls simply reuse the
running frame's key at about 304 ns. The 261 advances are at the frame
boundaries. The 670 stragglers at ~512 ns are the flip-flop packets
...
|-----|--------------------|-|-----|------------------|
| 4   | 3.32e-1 (332,443)  | | 40  | 1.00e-6 (1)      |
| 8   | 5.01e-2 (50,086)   | | 41  | 1.00e-6 (1)      |
| 12  | 1.10e-5 (11)       | | 42  | **0**            |
| 16  | 1.10e-5 (11)       | | 48  | 0                |
| 24  | 1.10e-5 (11)       | | 64  | 0                |
| 32  | 9.00e-6 (9)        | | 512 | 0                |
...
50 µs, and 50 µs/4.578 µs ≈ 11, so at most 11 later packets can
overtake it. Second, when a packet's path-A copy is lost, its path-B
copy arrives in its place 150 µs later, and 150 µs/4.578 µs ≈ 33, so
such a packet arrives about 33 positions late (22 to 44 once the two
copies' jitter is added). These two
numbers, 11 and 33, produce the three regions of the table.

- **Below K = 12 the losses are catastrophic.** Ordinary jitter
  reorders most packets a little. The network simulation counts the
  disorder it produces (saved in the raw output files under
  `../realistic_receiver/`):
  605,713 of the 1,000,000 packets (61%) arrived out of order, with a
  median lateness of 2 positions and a p99 of 9. A key window
  below these latenesses therefore loses a lot of packets.
- **From K = 12 to K = 41 only the 11 path-B rescues are lost.** They
...
|-------------|-------------------------|-------------------|----------|
| epoch       | 290.9 ns                | 316.1 ns          | +25 ns   |
| frame       | 301.4 ns                | 315.1 ns          | +14 ns   |
| packet      | 1046.7 ns               | 1134.0 ns         | +87 ns   |

The two benchmarks agree up to that receiver bookkeeping (14 to 87 ns
per call).
...
- `{epoch,frame,packet}_{clean,disturbed}.txt` hold the six main runs,
  ...
  attribution, timing).
- the K sweeps of section 4 are the `k_packet` and `k_frame` rows of
  `raw.csv` (written by `--sweep`), one row with the keying-loss rate
  per K value.
