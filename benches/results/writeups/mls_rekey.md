...
the rekey pipeline (see `key_derivation.md` for details).

## Discussion

For large broadcast groups (200-500 members), rekeying takes 2-5 ms; at
5000 members it reaches ~50-57 ms. This is fast enough for periodic per-epoch 
rekeys (during for example ad breaks).
